<script>
  ...
  // 金额统一走 lib/ccy 的 money(v, carrier, d)（报告币种口径的全局结构化入口，零 prop 接线）。
  import { money } from '../lib/ccy.svelte.js'

  let { rec, carrier } = $props()
  const mn = (v, d = 2) => money(v, carrier, d)
</script>

<div class="recon-line">
  {#if rec.partition}
    平台合计 {mn(rec.sum, 2)} {#if rec.ok}<span class="ok">＝</span>{:else}≠{/if} 公司总营收 {mn(rec.revenue, 2)} {#if rec.ok}<span class="ok">✓ 对账通过</span>{:else}（差 {mn(rec.diff, 2)}）{/if}
  {:else}
